...
function NavDrawerItem({ isActive, name }: { isActive: boolean; name: string }) {
  return (
    <li
      className={cn(
        'flex h-[28px] w-full items-center justify-start rounded-[4px] border border-transparent bg-[#2C2D36] pl-[14px] pr-2',
        isActive && 'border-white bg-[#15141A]',
      )}
    >
      <div className={cn('size-3 rounded-full bg-[#15141A]', isActive && 'bg-white')} />
      <div className={cn('pl-5 text-sm text-white/40', isActive && 'text-white')}>{name}</div>
    </li>
  );
}
